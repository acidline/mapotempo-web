class AddJsonDevicesColumnToVehicle < ActiveRecord::Migration
  
  def up

  	add_column :vehicles, :devices_linking, :jsonb

		Vehicle.order(:id).each{ |vehicle|

			vehicle.devices_linking = {
  ...
  def down
		...
		remove_column :vehicles, :devices_linking
